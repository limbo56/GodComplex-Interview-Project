...
public class ComposeMenu extends AbstractMenu {
  ...
  protected void initialize() {
    ...
    for (int i = 0; i < 5; i++) {
      if (i >= instrumentList.size()) return;
  ...
  private MenuItem getNoteItem(Instrument instrument, int index) {
    Material type = song.containsNote(instrument, index) ? Material.GLASS_PANE : Material.BEDROCK;
  ...
  private Runnable getNoteItemAction(Instrument instrument, int index) {
    return () -> {
      if (!song.containsNote(instrument, index)) song.addNote(instrument, index);
      else song.removeNote(instrument, index);
      open();
    };
  }
  ...
  private Runnable getInstrumentItemAction(Instrument instrument) {
    return () -> getOwner().playNote(getOwner().getLocation(), instrument, Note.natural(5, Tone.C));
  }
