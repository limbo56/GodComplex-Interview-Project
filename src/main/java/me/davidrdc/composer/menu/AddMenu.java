...
/**
 * This class represents an {@link AbstractMenu} that contains all the available {@link Instrument}s
 * that can be used when creating a {@link Song}
 *
    ...
    private Runnable getInstrumentAction(Instrument instrument) {
        return () -> {
            if (!composeMenu.getSong().hasInstrument(instrument)) {
                ...
                composeMenu.getSong().removeInstrument(instrument);
            }
            open();
        };
    }
